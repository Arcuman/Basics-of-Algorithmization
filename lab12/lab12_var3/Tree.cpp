...
namespace btree      // бинарное дерево, не допускается дублирование ключей
{
	...
	//-------------------------------
	void Node::scanDown1(void(*f)(void* n)) //Смешанный  (обратный) обход Л К П	
	{
		if (this->left != NULL)
	...
	int Node::printLevel(int level, void(*f)(void* n)) {
		if (this == nullptr) {
			// Если дерево пустое, то отображать нечего, выходим 
			return 1;
		}
		else {
			this->left ->printLevel(level - 1,f); // С помощью рекурсии посещаем левое поддерево 
			if (level == 0) {
				// level будет равен нулю на нужной глубине, так как при каждом рекурсивном вызове значение level уменьшается на один 
				f(this->data); // Показываем элемент, если он на нужном нам уровне 
			}
			this->right->printLevel(level - 1,f); // С помощью рекурсии посещаем правое поддерево 
		}
	}
	int Node::Height()
	{
	...
	void Node::scanDown(void(*f)(void* n)) //Нисходящий (прямой) обход К Л П
	{
		f(this->data);
		std::cout << std::endl;
		if (this->left != NULL)
